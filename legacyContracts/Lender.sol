...
contract Sender is CCIPReceiver, OwnerIsCreator {
  error FundsLocked(address msgSender, address token);
  error NotEnoughBalance(address msgSender, address token, uint256 balance, uint256 amountRequestedToBorrow, uint256 amountAvailableToBorrow);
  ...
  // parameters
  uint8 collateralizationRatio = 70;

  // constructor variables
  LinkTokenInterface linkToken;
  address borrowFactory;

  constructor(address _router, address link, address borrowFactoryAddress) CCIPReceiver(_router) {
    linkToken = LinkTokenInterface(link);
    borrowFactory = borrowFactoryAddress;
  }

  function lend(address tokenToStake, uint256 amount) external {
    IERC20(tokenToStake).transfer(address(this), amount);
    // TODO: do we need to record deposits if we can just check erc20 balance?
    deposits[tokenToStake].amount += amount;
  }

  function borrow(address tokenToBorrow, uint256 amount) external returns (bytes32 messageId) {
    if (deposits[tokenToBorrow].locked) {
      revert FundsLocked(msg.sender, tokenToBorrow);
    }
    uint256 memory availableToBorrow = deposits[tokenToBorrow].amount * 100 / collateralizationRatio;
    if (amount > availableToBorrow) {
      revert NotEnoughBalance(msg.sender, tokenToBorrow, deposits[tokenToBorrow].amount, amount, availableToBorrow);
    }
    deposits[tokenToStake].locked = true;

    // encode the depositor's EOA as  data to be sent in the message.
    bytes memory data = encode(msg.sender, tokenToBorrow, amount);

    Client.EVM2AnyMessage memory evm2AnyMessage = Client.EVM2AnyMessage({
      receiver: abi.encode(borrowFactory), // Sending to borrow factory
      data: data,
  ...
  function encode(address staker, address token, uint256 amount) public pure returns (string memory) {
    return string(abi.encodePacked(staker, token, amount));
  }

  function decode(bytes memory data) public pure returns (address staker, address token, uint256 amount) {
    require(data.length == 72, "Invalid encoded data length");
    assembly {
      staker := mload(add(data, 0x14))
      token := mload(add(data, 0x28))
      amount := mload(add(data, 0x48))
    }
  }
}
